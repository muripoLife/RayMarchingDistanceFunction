...
void main( void ) {
	...
	vec3 f1=vec3(1.0, 1.0, 1.0);
	vec3 f2=vec3(cPos.x, cPos.y - 1.0, cPos.z);
	// レイが進む処理(マーチングループ)
	for( float loop_num=.0; loop_num<16.; loop_num+=.05 ) {
		// 地形の式を書く.
        f1 = cPos += ray*loop_num;
		f2 = vec3(f1.x, f1.y - 1.0, f1.z);
		
		if( cos(f2.z) + sin(f2.x) > ++f2.y ) {
			if(f1.y-cos((cPos.x+cPos.z)*10.)>.7){
				ray = cUp;
			}else{
				ray = f1.x*cUp.yxz;
			}
